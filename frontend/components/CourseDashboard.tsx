...
import { Plus, Play, Link, Type } from 'lucide-react';

interface CourseDashboardProps {
  courses: Course[];
  onSelectCourse: (course: Course) => void;
  onAddCourse: (course: Partial<Course> & { url?: string }) => void;
  onNavigateToProfile: () => void;
}

const CourseDashboard: React.FC<CourseDashboardProps> = ({ courses, onSelectCourse, onAddCourse, onNavigateToProfile }) => {
  ...
  const handleCreateCourse = (e: React.FormEvent) => {
    ...
    onAddCourse({
      title: newCourseName || "New Course", 
      url: newCourseUrl,
  ...
  return (
    <div className="flex-1 w-full h-full overflow-y-auto custom-scrollbar p-8 lg:p-12 relative z-10">
      ...
      {/* Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-fade-in-up pt-4">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-[#161b22] border border-gray-700 w-full max-w-md rounded-2xl p-6 shadow-2xl scale-100 animate-in zoom-in-95 duration-200">
                <h2 className="text-xl font-bold text-white mb-4">Import New Course</h2>
                <form onSubmit={handleCreateCourse} className="space-y-4">
                    <div>
                        <label className="block text-sm text-gray-400 mb-1 flex items-center">
                            <Link size={14} className="mr-1"/> OneDrive Link (Required)
                        </label>
                        <input 
                            type="url" 
                    ...
                    <div>
                        <label className="block text-sm text-gray-400 mb-1 flex items-center">
                            <Type size={14} className="mr-1"/> Course Name (Optional)
                        </label>
                        <input 
                            type="text" 
                            className="w-full bg-[#0d1117] border border-gray-700 rounded-lg p-3 text-white focus:border-purple-500 focus:ring-1 focus:ring-purple-500 outline-none transition-all"
