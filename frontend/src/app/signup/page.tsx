...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: authError } = await signUp.email({
        email: email,
        password: password,
        name: username, // Better Auth usa 'name' invece di 'username'
      });
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      setError("Errore durante la registrazione con Google. Riprova più tardi.");
      setIsLoading(false);
    }
  };
  ...
  const handleGitHubSignUp = async () => {
    ...
    try {
      ...
      setError("Errore durante la registrazione con GitHub. Riprova più tardi.");
      setIsLoading(false);
    }
  };
          ...
          <div className="rounded-3xl bg-white/5 border border-white/10 p-8 backdrop-blur-xl shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button 
                ...
                disabled={isLoading}
                className="flex items-center justify-center gap-2 rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-white/80 transition-colors duration-150 hover:bg-white/10 hover:border-white/20 disabled:opacity-50 disabled:cursor-not-allowed"
              >
